...
function getClipUrl(search: string) {
  return `https://okizeme.b-cdn.net/dragunov/${encodeURIComponent(search)}.mp4`;
}
    ...
    <div className="mt-10 space-y-8">
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.9fr)] xl:items-start">
            <div className="grid gap-5 lg:grid-cols-2">
              {secrets.map((secret) => (
                <article
                  key={secret.title}
                  className="rounded-3xl border border-cyan-400/15 bg-white/80 p-6"
                >
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-cyan-600">
                    {secret.tag}
                  </p>
      ...
      {activeTab === "matchups" ? (
        <section className="space-y-6">
          ...
          {activeMatchup ? (
            <article className="rounded-3xl border border-cyan-300/20 bg-white/85 p-6 sm:p-8">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-cyan-600">
